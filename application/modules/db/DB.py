...
class DB:
    ...
    def insertMessage(self, data):
        query = 'INSERT INTO messages (user_id,message, name, room_id, time) VALUES (%s,%s, %s, %s, now())'
        self.cursor.execute(query, (data['userId'], data['message'], data['name'], data['roomId']))
        self.connect.commit()
        return True
